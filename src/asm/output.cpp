...
// List of symbols to put in the state file
static std::vector<Symbol *> stateSymbols;
...
void out_WriteState() {
	...
	FILE *file;
	if (stateFileName != "-") {
		file = fopen(stateFileName.c_str(), "w");
	} else {
		stateFileName = "<stdout>";
		file = fdopen(STDOUT_FILENO, "w");
	}
	if (!file)
		err("Failed to open state file '%s'", stateFileName.c_str());
	Defer closeFile{[&] { fclose(file); }};

	// Only write the non-builtin numeric constants
	sym_ForEach([](Symbol &sym) {
		if (!sym.isBuiltin && sym.type == SYM_EQU)
			stateSymbols.push_back(&sym);
	});
	// Symbols are ordered by file, then by line number, then by name
	std::sort(RANGE(stateSymbols), [](Symbol *sym1, Symbol *sym2) -> bool {
		return sym1->key < sym2->key;
	});

	fputs("; File generated by rgbasm\n", file);

	for (Symbol const *sym : stateSymbols) {
		uint32_t value = static_cast<uint32_t>(sym->getOutputValue());
		if (value < 0x100)
			fprintf(file, "%02" PRIx32 " ", value);
		else if (value < 0x10000)
			fprintf(file, "%04" PRIx32 " ", value);
		else
...
void out_SetStateFileName(std::string const &name) {
	if (!stateFileName.empty())
		warnx("Overriding state filename %s", stateFileName.c_str());
	stateFileName = name;
	if (verbose)
