...
static bool isValidDigit(char c, int canonical, int base) {
	return (c >= '0' && c <= '9' && (c - '0' >= base || c - '0' == canonical))
	       || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '.' || c == '#' || c == '@';
}

void lexer_SetBinDigits(char const digits[2]) {
	for (int i = 0; i < 2; i++) {
		if (char c = digits[i]; !isValidDigit(c, i, 2)) {
			error("Invalid digit for binary constant %s\n", printChar(c));
			return;
		}
	}
...
void lexer_SetGfxDigits(char const digits[4]) {
	for (int i = 0; i < 4; i++) {
		if (char c = digits[i]; !isValidDigit(c, i, 4)) {
			error("Invalid digit for graphics constant %s\n", printChar(c));
			return;
		}
	}
