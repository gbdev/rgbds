...
These files are in the `src/` directory. They are shared across multiple programs: often all four of them, RGBASM, RGBLINK, RGBFIX, and RGBGFX, sometimes just RGBASM and RGBLINK.
...
These source files are in the `src/extern/` directory. They have been copied ("vendored") from external authors and adapted for use with RGBDS. Both of our vendored dependencies use the same MIT license as RGBDS.
